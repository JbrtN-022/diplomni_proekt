...
namespace rccs_new
{
    ...
    public partial class форма_администратора : Window
    {
        public форма_администратора()
        {
            ...
            HistoryLogger.Log(
                $"Пользователь {ConnectionBD.resFio} авторизовался как администратор");
        }
...
",
        ...
        {
            форма_пользователей polzovateli =
                new форма_пользователей();
            ...
            HistoryLogger.Log(
                $"Пользователь {ConnectionBD.resFio} открыл форму списка пользователей");
        ...
        {
            форма_справочников spravochnik =
                new форма_справочников();
            ...
            HistoryLogger.Log(
                $"Пользователь {ConnectionBD.resFio} открыл форму списка справочников");
        ...
        {
            форма_контрагентов form =
                new форма_контрагентов();
            ...
            HistoryLogger.Log(
                $"Пользователь {ConnectionBD.resFio} открыл форму списка контрагентов");
        ...
        private void прайс_Click(object sender, RoutedEventArgs e)
        {
            ...
            HistoryLogger.Log(
                $"Пользователь {ConnectionBD.resFio} открыл форму списка истории прайслиста");
        ...
        private void пользователь_Click(object sender, RoutedEventArgs e)
        {
            ...
            HistoryLogger.Log(
                $"Пользователь {ConnectionBD.resFio} открыл форму данных пользователя");
        }

        private void документы_Click(object sender, RoutedEventArgs e)
        {
            форма_договоров__лицензий form =
                new форма_договоров__лицензий();
            ...
            HistoryLogger.Log(
                $"Пользователь {ConnectionBD.resFio} открыл форму договора лицензий");
        ...
        private void экспорт_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                if (saveDialog.ShowDialog() == true)
                {
                    ...
                    HistoryLogger.Log(
                        $"Пользователь {ConnectionBD.resFio} экспортировал базу данных. Файл: {saveDialog.FileName}");
            ...
            {
                HistoryLogger.Log(
                    $"Ошибка экспорта базы данных: {ex.Message}");
        ...
        private void импорт_Click(object sender, RoutedEventArgs e)
        {
                ...
                if (openDialog.ShowDialog() == true)
                {
                    ...
                    foreach (string table in requiredTables)
                    {
                        ...
                        if (!tableExists)
                        {
                            ...
                            MessageBox.Show(
                                $"В файле отсутствует таблица: {table}",
                                "Ошибка импорта",
                                MessageBoxButton.OK,
                    ...
                    {
                        HistoryLogger.Log(
                            $"Пользователь {ConnectionBD.resFio} попытался импортировать неверную базу данных");
                    ...
                    HistoryLogger.Log(
                        $"Пользователь {ConnectionBD.resFio} импортировал базу данных из файла: {openDialog.FileName}");
            ...
            {
                HistoryLogger.Log(
                    $"Ошибка импорта базы данных: {ex.Message}");
